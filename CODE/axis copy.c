...
void fifo_handler() {
    ...
    if ( int_read_back & ( BIT_MSG_DMP_INT | BIT_MSG_DMP_INT_0 | BIT_MSG_DMP_INT_2 | BIT_MSG_DMP_INT_5 ) ) {
        ...
        do {
            ...
            sample_nb = 0;
            if ( sample_cnt_array[ ANDROID_SENSOR_ACCELEROMETER ] ) sample_nb++;
            if ( sample_cnt_array[ ANDROID_SENSOR_GYROSCOPE ] ) sample_nb++;
            if ( sample_cnt_array[ ANDROID_SENSOR_GAME_ROTATION_VECTOR ] ) sample_nb++;
            ...
            while ( total_sample_cnt-- ) {
                ...
                if ( header & GYRO_SET ) {
                    ...
                    if ( ( bias_data[ 0 ] != 0 ) && ( bias_data[ 1 ] != 0 ) && ( bias_data[ 2 ] != 0 ) ) gyro_accuracy = 3;
#endif
                }  // header & GYRO_SET
